...
struct Host {
  ...
  Host() =delete;
  Host(float d, float p, Symbiont s, int id, float vert_trans) : donation(d), points(p), sym(s), cell_id(id), vert_trans(vert_trans) {};
  Host(float d, Symbiont s, int id, float vert_trans) : donation(d), points(0.0), sym(s), cell_id(id), vert_trans(vert_trans) {};
  Host(const Host &orig) : donation(orig.donation), points(orig.points), cell_id(orig.cell_id), sym(orig.sym), vert_trans(orig.vert_trans) {};
...
void Host::mutate(std::mt19937& r, double rate){
  ...
  vert_trans+= dist(r);
  if (vert_trans > 1) vert_trans = 1;
  if (vert_trans < 0) vert_trans = 0;
...
void Population::print_stats() {
  ...
  std::vector<int> vert_trans_rates(11, 0);


  for(auto org : pop){
    host_count++;
    host_sum += org.donation;
    vert_trans_sum += org.vert_trans;
    int vert_trans_val = (org.vert_trans) * 10;
    vert_trans_rates[vert_trans_val] += 1;
  ...
  data_file << cur_update <<", "<< host_sum/host_count << ", " << sym_sum/sym_count << ", " << host_count << ", " << sym_count << ", " << vert_trans_sum/host_count << endl << std::flush;
  ...
  std::copy(vert_trans_rates.begin(), vert_trans_rates.end(), std::ostream_iterator<int>(vert_trans_file, ", "));
  vert_trans_file << endl << std::flush;
  ...
  for(cur_update = 0; cur_update < final_update; ++cur_update){
	...
	if(pop[infected].sym.donation == -2)
      ...
      if (org.points >=(10*rec_res)){
        ...
        Host baby(org.donation, 0.0, Symbiont(), -2, org.vert_trans);
        if(org.sym.points > 0 && dist(engine) < org.vert_trans){
